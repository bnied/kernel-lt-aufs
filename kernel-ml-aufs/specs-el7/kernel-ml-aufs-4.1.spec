...
BuildRequires: net-tools, openssl, patch >= 2.5.4, perl, redhat-rpm-config >= 9.1.0-55
BuildRequires: sh-utils, tar, xmlto, xz
%if %{with_perf}
BuildRequires: audit-libs-devel, binutils-devel, bison elfutils-devel, newt-devel
BuildRequires: numactl-devel, perl(ExtUtils::Embed), python-devel, slang-devel, zlib-devel
%endif
%if %{with_tools}
...
Source3: ../configs-el7/config-%{version}-x86_64
Source4: ../configs-el7/cpupower.service
Source5: ../configs-el7/cpupower.config
Source6: ../%{AUFSver}.tar
...
Provides: kernel-ml-doc = %{version}-%{release}
...
Provides: kernel-ml-headers = %{version}-%{release}
...
%if %{with_perf}
%global perf_make \
    %{__make} -s -C tools/perf %{?_smp_mflags} V=1 WERROR=0 NO_LIBUNWIND=1 HAVE_CPLUS_DEMANGLE=1 NO_GTK2=1 NO_STRLCPY=1 prefix=%{_prefix}

%{perf_make} all
%{perf_make} man 2> /dev/null || false
%endif
...
/usr/bin/find $RPM_BUILD_ROOT/usr/include -type f \( -name .install -o -name .check -o -name ..install.cmd -o -name ..check.cmd \) | \
    xargs --no-run-if-empty %{__rm} -f
%endif

%if %{with_doc}
DOCDIR=$RPM_BUILD_ROOT%{_datadir}/doc/kernel-ml-doc-%{version}
...
mkdir -p $MAN9DIR
/usr/bin/find Documentation/DocBook/man -type f -name '*.9.gz' -print0 | xargs -0 --no-run-if-empty %{__install} -m 444 -t $MAN9DIR $m
...
%if %{with_default}
%posttrans
%{_sbindir}/new-kernel-pkg --package kernel-ml --mkinitrd --dracut --depmod --update %{version}-%{release}.%{_target_cpu} || exit $?
%{_sbindir}/new-kernel-pkg --package kernel-ml --rpmposttrans %{version}-%{release}.%{_target_cpu} || exit $?
if [ -x %{_sbindir}/weak-modules ]; then
    %{_sbindir}/weak-modules --add-kernel %{version}-%{release}.%{_target_cpu} || exit $?
fi

%post
%{_sbindir}/new-kernel-pkg --package kernel-ml --install %{version}-%{release}.%{_target_cpu} || exit $?
...
%if %{with_tools}
%post -n kernel-ml-tools
%{_sbindir}/ldconfig || exit $?

%postun -n kernel-ml-tools
%{_sbindir}/ldconfig || exit $?
%endif
...
%files doc
%defattr(-,root,root)
%{_datadir}/doc/kernel-ml-doc-%{version}/Documentation/*
%dir %{_datadir}/doc/kernel-ml-doc-%{version}/Documentation
%dir %{_datadir}/doc/kernel-ml-doc-%{version}
%{_datadir}/man/man9/*
%endif
...
%if %{with_tools}
%files -n kernel-ml-tools -f cpupower.lang
%defattr(-,root,root)
...
%files -n kernel-ml-tools-libs
%defattr(-,root,root)
%{_libdir}/libcpupower.so.0
%{_libdir}/libcpupower.so.0.0.0

%files -n kernel-ml-tools-libs-devel
%defattr(-,root,root)
...
- CONFIG_BLK_DEV_NBD=m [http://elrepo.org/bugs/view.php?id=538]
...
- CONFIG_BLK_DEV_RBD=m [http://elrepo.org/bugs/view.php?id=521]
