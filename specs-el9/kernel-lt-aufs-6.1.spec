...
#
# Additional option for toracat-friendly, one-off, kernel-lt-aufs building.
# Only build the base kernel-lt-aufs (--with baseonly):
%define with_baseonly     %{?_with_baseonly:        1} %{?!_with_baseonly:        0}

%global KVERREL %{pkg_version}-%{pkg_release}.%{_target_cpu}

# If requested, only build base kernel-lt-aufs package.
%if %{with_baseonly}
%define with_doc 0
%define with_perf 0
%define with_tools 0
...
#
# List the packages required for the kernel-lt-aufs build.
#
...
#
# This macro does requires, provides, conflicts, obsoletes for a kernel-lt-aufs package.
...
Provides: installonlypkg(kernel)\
Provides: installonlypkg(kernel-lt-aufs)\
AutoReqProv: no\
...
#
# This macro creates an empty kernel-lt-aufs-<subpackage>-devel-matched package that
...
Provides: installonlypkg(kernel-lt-aufs-module)\
...
#
# This macro creates a kernel-lt-aufs-<subpackage> and its -devel.
#   %%define variant_summary The Linux kernel-lt-aufs compiled for <configuration>
...
# Files for 'make scripts' to succeed with kernel-lt-aufs-devel.
...
# Files for 'make prepare' to succeed with kernel-lt-aufs-devel.
...
%ifarch x86_64
# Files required for 'make prepare' to succeed with kernel-lt-aufs-devel.
...
# Identify modules in the kernel-lt-aufs-modules-extras package
...
# Don't include anything going into kernel-lt-aufs-modules-extra in the file lists.
xargs rm -fr < mod-extra.list

# Find all the module files and filter them out into the core and modules lists.
# This actually removes anything going into kernel-lt-aufs-modules from the directory.
...
# for when the kernel-lt-aufs-devel package is installed.
ln -sf /usr/src/kernels/%{KVERREL} $RPM_BUILD_ROOT/lib/modules/%{KVERREL}/build

# Move the generated vmlinux.h file into the kernel-lt-aufs-devel directory structure.
...
# kernel-lt-aufs headers_install will remove any header files in /usr/include that
# it doesn't install itself.

%if %{with_headers}
# Install kernel-lt-aufs headers
...
#
# This macro defines a %%post script for a kernel-lt-aufs*-devel package.
...
#
# This macro defines a %%post script for a kernel-lt-aufs*-modules-extra package.
...
#
# This macro defines a %%post script for a kernel-lt-aufs*-modules package.
# It also defines a %%postun script that does the same thing.
...
# This macro defines a %%posttrans script for a kernel-lt-aufs package.
...
#
# This macro defines the %%files sections for a kernel-lt-aufs package
...
- The fourth release candidate of a kernel-lt-aufs package set for el9.
...
- The third release candidate of a kernel-lt-aufs package set for el9.

* Tue Sep 20 2022 Alan Bartlett <ajb@elrepo.org> - 5.19.10-0.rc2
- [https://www.kernel.org/pub/linux/kernel/v5.x/ChangeLog-5.19.10]
- The second release candidate of a kernel-lt-aufs package set for el9.

* Sun Sep 18 2022 Alan Bartlett <ajb@elrepo.org> - 5.19.10-0.rc1
- Updated with the 5.19.10 source tarball.
- [https://www.kernel.org/pub/linux/kernel/v5.x/ChangeLog-5.19.10]
- The first release candidate of a kernel-lt-aufs package set for el9.

* Sat Aug 13 2022 Alan Bartlett <ajb@elrepo.org>
- Forked this specification file to create a kernel-lt-aufs package set for the ELRepo Project.
